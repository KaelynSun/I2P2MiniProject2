...
void Slider::Draw() const {
    ...
    Bar.Draw(); // Line 
    End1.Draw(); // Dot at the start & end
    End2.Draw();
    ImageButton::Draw(); // Knob
}
...
void Slider::SetValue(float value) {
    ...
    this->value = std::clamp(value, Min, Max); // Interval
    Position.x = Bar.Position.x + (Bar.Size.x * this->value); // Update knob position
    if (OnValueChangedCallback) { // Keep
